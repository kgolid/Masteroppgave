...
It turns out that none of the vertices in the graph satisfies this requirement; therefore $a$ and $b$ are not $V_3$-related, so implication (2) fails.

\subsection{The road ahead}
\label{sub:The road ahead}
The inductive part of the $V_2$-definition provided a proof step where one of the NAND-clauses in the premise always was axiomatic.
As a result of removing this constraint from the proof step, we got a more general vel-definition, $V_3$.

The question now is thefore ``What is left to generalize?''.

...


While generalizing our graph structural definitions, we have reached a variant that bears a great resemblance to the rules in our proof system; more specifically, the rules that conclude with binary NAND-clauses.
