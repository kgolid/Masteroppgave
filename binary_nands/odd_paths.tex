...
It is however easy to find an example where the implication does not hold in the opposite direction.\par
\begin{figure}[!h]
  \centering
  \begin{tikzpicture}
    [
...
In other words, given any path, if one of its vertices is pointing to its successor in the path only, that vertex is trimmed in that path.
...
With the above proof in mind, consider the following graph:\par
\begin{figure}[!h]
  \centering
  \begin{tikzpicture}
    [
...
A path $\langle x_0,x_k\rangle$\todo{This notation is not explained} is oddly trimmed if for each odd $i < k$, $x_i$ is trimmed with respect to that path.
...
We can thus generalize further and say that any two vertices connected by an oddly trimmed path of odd length are vel-connected.
