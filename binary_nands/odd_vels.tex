...
The formal proof why all NAND-clauses from vertices connected by such vels are provable in Neg can be found in Appendix~\ref{sec:Provability of NAND-clauses from vels}.

There are obviously other ways of altering directions of individual edges in a path.
The next example will however show that most of them will alter the OR-clauses in such a way that implication (1) no longer holds.
...
In both examples, the shown kernels contain both $a$ and $b$, showing that neither gives a provable NAND-clause $\ol{ab}$ in Neg.
