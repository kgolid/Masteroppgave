% !TEX root= ../main.tex
\externdaldocument{../binary_nands/first_inductive}
...
We consider the graph in Figure~\ref{fig:v3_counter_graph} from Section~\ref{sub:V3 and implication 2}, shown below:\par
\begin{figure}[!h]
  \centering
  \begin{tikzpicture}
    [
  ...
  \label{fig:open_door}
\end{figure}
As before, the vertices $y_1$, $y_2$ and $c_2$ are still initial vertices of disjoint rays, and not sinks.
...
The NAND-clause that we want to prove is $\ol{ab}$, and this is one way to prove it in Neg:\par
\begin{figure}[!h]
  \centering
  \begin{prooftree*}
    \Hypo{\ol{ax_2}}
  ...
  \end{prooftree*}
  \caption{}
  \label{fig:open_door_proof}
\end{figure}
The premise of the last rule application in the proof above contains two non-binary NAND-clauses.
We will show that this is unavoidable.

By taking a closer look at the graph, we can work out these 6 solutions, among others (black vertices being assigned 1, while white vertices being assigned 0):\par
\begin{figure}[!h]
  \centering
  \begin{tikzpicture}
    [
