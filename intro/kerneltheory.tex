...
It is shown in \cite{cook} that every digraph (with at least one edge) can be transformed into a infinitary dag such that $\alpha$ is a solution to the created dag if and only if it is a solution to the original digraph.
